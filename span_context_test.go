...
func TestIsValidFromHex(t *testing.T) {
	for _, testcase := range []struct {
		...
		{
			name:  "Valid TraceID",
			tid:   otel.ID([16]byte{128, 241, 152, 238, 86, 52, 59, 168, 100, 254, 139, 42, 87, 211, 239, 247}),
		...
		t.Run(testcase.name, func(t *testing.T) {
			tid, err := otel.IDFromHex(testcase.hex)
...
func TestSpanContextIsSampled(t *testing.T) {
	for _, testcase := range []struct {
	...
	}{
		{
			...
			sc: otel.SpanContext{
				TraceID:    otel.ID([16]byte{1}),
				TraceFlags: otel.FlagsSampled | ^otel.FlagsSampled,
			},
			want: true,
		}, {
			name: "not sampled/default",
			sc:   otel.SpanContext{TraceID: otel.ID{}},
			want: false,
		},
	} {
		t.Run(testcase.name, func(t *testing.T) {
...
func TestStringSpanID(t *testing.T) {
	for _, testcase := range []struct {
	...
	}{
		{
			...
			sid:  otel.SpanID([8]byte{255}),
			want: "ff00000000000000",
		},
		{
			name: "SpanID.String returns string representation of self.SpanID values == 0",
			sid:  otel.SpanID([8]byte{}),
