...
func TestRecordingIsOff(t *testing.T) {
	tp, _ := NewProvider()
	_, span := tp.NewTracer("Recording off").Start(context.Background(), "StartSpan")
	defer span.End()
...
func TestStartSpanWithChildOf(t *testing.T) {
	tp, _ := NewProvider()
	tr := tp.NewTracer("SpanWith ChildOf")
...
func TestStartSpanWithFollowsFrom(t *testing.T) {
	tp, _ := NewProvider()
	tr := tp.NewTracer("SpanWith FollowsFrom")
...
func TestSetSpanName(t *testing.T) {
	...
	_, span := tp.NewTracer("SetSpanName").Start(context.Background(), "SpanName-1",
...
func startNamedSpan(tp *Provider, trName, name string, args ...apitrace.SpanOption) apitrace.Span {
	...
	_, span := tp.NewTracer(trName).Start(
		context.Background(),
		name,
		args...,
	)
...
func TestExecutionTracerTaskEnd(t *testing.T) {
	...
	tr := tp.NewTracer("Execution Tracer Task End")
...
func TestCustomStartEndTime(t *testing.T) {
	...
	_, span := tp.NewTracer("Custom Start and End time").Start(
		context.Background(),
		"testspan",
