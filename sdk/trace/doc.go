...
samplers. It is ignored if OTEL_TRACES_SAMPLER is not set. Values that cannot
be parsed as a number, or that are less than 0 or greater than 1, are reported
to the global error handler, and the default is used instead.
The configuration can be overridden by the [WithSampler] option.
...
the spans it holds. Values that cannot be parsed as an integer are ignored,
and the default is used instead. Other values are used as-is; valid values are
positive.
...
an export to complete. Values that cannot be parsed as an integer are ignored,
and the default is used instead. Other values are used as-is; a value of zero
or less disables the timeout.
...
processor returned from [NewBatchSpanProcessor] queues for export. Values that
cannot be parsed as an integer are ignored, and the default is used instead.
Other values are used as-is; valid values are positive.
...
Values that cannot be parsed as an integer are ignored, and the default is used
instead. Other values are used as-is; valid values are positive. If the value
is greater than OTEL_BSP_MAX_QUEUE_SIZE, the smaller of 512 and
OTEL_BSP_MAX_QUEUE_SIZE is used instead.
...
OTEL_ATTRIBUTE_VALUE_LENGTH_LIMIT is used. Values that cannot be parsed as an
integer are ignored, and the default is used instead. Other values are used
as-is; a negative value means no limit.
...
set. Values that cannot be parsed as an integer are ignored, and the default is
used instead. Other values are used as-is; a negative value means no limit.
The configuration can be overridden by the [WithRawSpanLimits] option.
...
[SpanLimits]. If it is not set, OTEL_ATTRIBUTE_COUNT_LIMIT is used. Values that
cannot be parsed as an integer are ignored, and the default is used instead.
Other values are used as-is; a negative value means no limit.
The configuration can be overridden by the [WithRawSpanLimits] option.

OTEL_ATTRIBUTE_COUNT_LIMIT (default: "128") - the maximum allowed span
attribute count, used when OTEL_SPAN_ATTRIBUTE_COUNT_LIMIT is not set. Values
that cannot be parsed as an integer are ignored, and the default is used
instead. Other values are used as-is; a negative value means no limit.
The configuration can be overridden by the [WithRawSpanLimits] option.
...
AttributePerEventCountLimit field of [SpanLimits]. Values that cannot be parsed
as an integer are ignored, and the default is used instead. Other values are
used as-is; a negative value means no limit.
The configuration can be overridden by the [WithRawSpanLimits] option.
...
AttributePerLinkCountLimit field of [SpanLimits]. Values that cannot be parsed
as an integer are ignored, and the default is used instead. Other values are
used as-is; a negative value means no limit.
The configuration can be overridden by the [WithRawSpanLimits] option.
*/
