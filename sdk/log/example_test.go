...
// Use a processor which sets EventName on log records having "otel.event_name" string attribute.
...
// on log records having "otel.event_name" string attribute.
...
// OnEmit sets the EventName on log records having an "otel.event_name" string attribute.
...
func (p *EventNameProcessor) OnEmit(_ context.Context, record *log.Record) error {
	record.WalkAttributes(func(kv logapi.KeyValue) bool {
		if kv.Key == "otel.event_name" && kv.Value.Kind() == logapi.KindString {
