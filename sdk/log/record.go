...
func (r *Record) WalkAttributes(f func(log.KeyValue) bool) {
	...
	for i := 0; i < r.nFront; i++ {
		if !f(r.front[i]) { // Warning: this can lead to deadlock
			return
		}
	}
	for _, a := range r.back {
		if !f(a) { // Warning: this can lead to deadlock
			return
		}
	}
}
...
func (r *Record) Clone() *Record {
	r.mu.Lock()
	defer r.mu.Unlock()
	res := *r //nolint // Mutex is reinitialized below.
	res.mu = sync.Mutex{}
