...
// simpleLogRecordProcessor is a LogRecordProcessor that synchronously sends all
...
// send completed logRecords to the exporter immediately.
//
...
func NewSimpleLogRecordProcessor(exporter LogRecordExporter) LogRecordProcessor {
	ssp := &simpleLogRecordProcessor{
		exporter: exporter,
	}
	return ssp
}

func (ssp *simpleLogRecordProcessor) OnEmit(ctx context.Context, s ReadWriteLogRecord) {
	ssp.exporterMu.RLock()
	defer ssp.exporterMu.RUnlock()

	if ssp.exporter != nil {
		if err := ssp.exporter.ExportLogRecords(context.Background(), []ReadOnlyLogRecord{s}); err != nil {
			otel.Handle(err)
		}
	}
}
...
func (ssp *simpleLogRecordProcessor) Shutdown(ctx context.Context) error {
	var err error
	ssp.stopOnce.Do(
		func() {
			stopFunc := func(exp LogRecordExporter) (<-chan error, func()) {
				done := make(chan error)
				return done, func() { done <- exp.Shutdown(ctx) }
			}

			// The exporter field of the simpleLogRecordProcessor needs to be zeroed to
			...
			// field is zeroed. This ensures the simpleLogRecordProcessor is shut down
...
func (ssp *simpleLogRecordProcessor) ForceFlush(context.Context) error {
	return nil
}

// MarshalLog is the marshaling function used by the logging system to represent this Span Processor.
func (ssp *simpleLogRecordProcessor) MarshalLog() interface{} {
	return struct {
		Type     string
		Exporter LogRecordExporter
	}{
