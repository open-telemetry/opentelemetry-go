...
func TestRecordBody(t *testing.T) {
	testcases := []struct {
		...
		{
			name: "bytes value",
			body: log.ByteSliceValue([]byte("test")),
			want: log.ByteSliceValue([]byte("test")),
		},
		{
			name: "empty slice",
			body: log.SliceValue(),
...
func TestApplyAttrLimitsTruncation(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			name:  "ByteSlice",
			limit: 0,
			input: log.ByteSliceValue([]byte("foo")),
			want:  log.ByteSliceValue([]byte("foo")),
		},
		{
			name:  "String",
			limit: 0,
		...
		{
			name:  "LongByteSliceNotTruncated",
			limit: 5,
			input: log.ByteSliceValue([]byte("This is a very long byte array")),
			want:  log.ByteSliceValue([]byte("This is a very long byte array")),
		},
		{
...
func BenchmarkSetBody(b *testing.B) {
	...
	uniqueMapValue := log.MapValue(
		...
		log.ByteSlice("bytes_key", []byte("data")),
	)
