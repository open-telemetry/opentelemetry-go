...
var _ aggregator.MinMaxSumCount = &AggregatorSL{}

// NewSL returns a new measure aggregator for computing min, max, sum, and
...
func NewSL(desc *metric.Descriptor) *AggregatorSL {
...
func (c *AggregatorSL) Min() (core.Number, error) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
// The error value aggregator.ErrNoData will be returned
// if there were no measurements recorded during the checkpoint.
func (c *AggregatorSL) Max() (core.Number, error) {
	c.lock.Lock()
	defer c.lock.Unlock()
	if c.checkpoint().count.IsZero(core.Uint64NumberKind) {
		return c.kind.Zero(), aggregator.ErrNoData
