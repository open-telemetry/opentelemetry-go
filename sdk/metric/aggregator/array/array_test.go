...
func (ut *updateTest) run(t *testing.T, profile aggregatortest.Profile) {
	descriptor := aggregatortest.NewAggregatorTest(otel.ValueRecorderKind, profile.NumberKind)
	agg, ckpt := new2()
...
func TestArrayFloat64(t *testing.T) {
	descriptor := aggregatortest.NewAggregatorTest(otel.ValueRecorderKind, otel.Float64NumberKind)
