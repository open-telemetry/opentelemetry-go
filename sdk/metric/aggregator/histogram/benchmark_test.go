...
func benchmarkHistogramSearchFloat64(b *testing.B, size int) {
	...
	desc := aggregatortest.NewAggregatorTest(otel.ValueRecorderKind, otel.Float64NumberKind)
...
func benchmarkHistogramSearchInt64(b *testing.B, size int) {
	...
	desc := aggregatortest.NewAggregatorTest(otel.ValueRecorderKind, otel.Int64NumberKind)
	agg := &histogram.New(1, desc, boundaries)[0]
	ctx := context.Background()
