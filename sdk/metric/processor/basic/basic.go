...
var ErrInvalidTemporality = fmt.Errorf("invalid export kind")
...
func New(aselector export.AggregatorSelector, eselector export.TemporalitySelector, opts ...Option) *Processor {
	return NewFactory(aselector, eselector, opts...).NewCheckpointer().(*Processor)
}

type factory struct {
	aselector export.AggregatorSelector
	eselector export.TemporalitySelector
	config    config
}

func NewFactory(aselector export.AggregatorSelector, eselector export.TemporalitySelector, opts ...Option) export.CheckpointerFactory {
	var config config
...
func (f factory) NewCheckpointer() export.Checkpointer {
	...
	p := &Processor{
		...
		TemporalitySelector: f.eselector,
		state: state{
			values:        map[stateKey]*stateValue{},
			processStart:  now,
			intervalStart: now,
...
// to an exporter.
func (b *state) ForEach(exporter export.TemporalitySelector, f func(export.Record) error) error {
