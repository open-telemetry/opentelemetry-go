	...
	for _, tc := range []exportCase{
		...
		t.Run(tc.kind.String(), func(t *testing.T) {
			for _, ic := range []instrumentCase{
				{kind: otel.CounterKind},
				{kind: otel.UpDownCounterKind},
				{kind: otel.ValueRecorderKind},
				{kind: otel.SumObserverKind},
				{kind: otel.UpDownSumObserverKind},
				{kind: otel.ValueObserverKind},
			} {
...
func TestBasicInconsistent(t *testing.T) {
	...
	desc := otel.NewDescriptor("inst", otel.CounterKind, otel.Int64NumberKind)
...
func TestStatefulNoMemoryCumulative(t *testing.T) {
	...
	desc := otel.NewDescriptor("inst.sum", otel.CounterKind, otel.Int64NumberKind)
	selector := processorTest.AggregatorSelector()
...
func TestStatefulNoMemoryDelta(t *testing.T) {
	...
	desc := otel.NewDescriptor("inst.sum", otel.SumObserverKind, otel.Int64NumberKind)
	selector := processorTest.AggregatorSelector()
...
func TestMultiObserverSum(t *testing.T) {
	for _, ekind := range []export.ExportKind{
		...
		res := resource.New(label.String("R", "V"))
		desc := otel.NewDescriptor("observe.sum", otel.SumObserverKind, otel.Int64NumberKind)
		selector := processorTest.AggregatorSelector()
