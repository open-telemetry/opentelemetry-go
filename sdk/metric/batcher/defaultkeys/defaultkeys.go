...
func (b *Batcher) Process(_ context.Context, record export.Record) error {
	...
	encoded := b.labelEncoder.Encode(export.NewSliceLabelIterator(outputLabels))
	...
	b.aggCheckpoint[key] = export.NewRecord(
		desc,
		export.NewLabels(export.NewSliceLabelIterator(outputLabels), encoded, b.labelEncoder),
		agg,
	)
	return nil
}
