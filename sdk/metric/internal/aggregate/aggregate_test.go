...
func TestInt64Precision(t *testing.T) {
	ctx := t.Context()
	const (
		largeVal int64 = (1 << 53) + 1
		incVal   int64 = 1
	)

	t.Run("DeltaSum", func(t *testing.T) {
		meas, comp := Builder[int64]{Temporality: metricdata.DeltaTemporality}.Sum(true)
		var agg metricdata.Aggregation

		meas(ctx, largeVal, alice)
		require.Equal(t, 1, comp(&agg))
		sum := agg.(metricdata.Sum[int64])
		require.Len(t, sum.DataPoints, 1)
		assert.Equal(t, largeVal, sum.DataPoints[0].Value)

		meas(ctx, incVal, alice)
		require.Equal(t, 1, comp(&agg))
		sum = agg.(metricdata.Sum[int64])
		require.Len(t, sum.DataPoints, 1)
		assert.Equal(t, incVal, sum.DataPoints[0].Value)
	})

	t.Run("CumulativeSum", func(t *testing.T) {
		meas, comp := Builder[int64]{Temporality: metricdata.CumulativeTemporality}.Sum(true)
		var agg metricdata.Aggregation
		...
		sum = agg.(metricdata.Sum[int64])
		require.Len(t, sum.DataPoints, 1)
		assert.Equal(t, largeVal+incVal, sum.DataPoints[0].Value)
	})

	t.Run("DeltaPrecomputedSum", func(t *testing.T) {
		meas, comp := Builder[int64]{Temporality: metricdata.DeltaTemporality}.PrecomputedSum(true)
		var agg metricdata.Aggregation
		...
		meas(ctx, largeVal+incVal, alice)
		require.Equal(t, 1, comp(&agg))
		sum = agg.(metricdata.Sum[int64])
		require.Len(t, sum.DataPoints, 1)
		assert.Equal(t, incVal, sum.DataPoints[0].Value)
	...
	t.Run("CumulativePrecomputedSum", func(t *testing.T) {
		meas, comp := Builder[int64]{Temporality: metricdata.CumulativeTemporality}.PrecomputedSum(true)
		var agg metricdata.Aggregation
	...
	t.Run("DeltaFinishSum", func(t *testing.T) {
		fs := Builder[int64]{Temporality: metricdata.DeltaTemporality}.FinishSum(true)
		var agg metricdata.Aggregation

		fs.Measure(ctx, largeVal, alice)
		require.Equal(t, 1, fs.ComputeAggregation(&agg))
		sum := agg.(metricdata.Sum[int64])
		require.Len(t, sum.DataPoints, 1)
		assert.Equal(t, largeVal, sum.DataPoints[0].Value)

		fs.Measure(ctx, incVal, alice)
		require.Equal(t, 1, fs.ComputeAggregation(&agg))
		sum = agg.(metricdata.Sum[int64])
		require.Len(t, sum.DataPoints, 1)
		assert.Equal(t, incVal, sum.DataPoints[0].Value)
	...
	t.Run("CumulativeFinishSum", func(t *testing.T) {
		fs := Builder[int64]{Temporality: metricdata.CumulativeTemporality}.FinishSum(true)
		var agg metricdata.Aggregation
	...
	t.Run("DeltaHistogram", func(t *testing.T) {
		...
		meas(ctx, largeVal, alice)
		require.Equal(t, 1, comp(&agg))
		hist := agg.(metricdata.Histogram[int64])
		require.Len(t, hist.DataPoints, 1)
		assert.Equal(t, largeVal, hist.DataPoints[0].Sum)

		meas(ctx, incVal, alice)
		require.Equal(t, 1, comp(&agg))
		hist = agg.(metricdata.Histogram[int64])
		require.Len(t, hist.DataPoints, 1)
		assert.Equal(t, incVal, hist.DataPoints[0].Sum)
	})

	t.Run("CumulativeHistogram", func(t *testing.T) {
		meas, comp := Builder[int64]{
		...
		hist = agg.(metricdata.Histogram[int64])
		require.Len(t, hist.DataPoints, 1)
		assert.Equal(t, largeVal+incVal, hist.DataPoints[0].Sum)
	})

	t.Run("DeltaExponentialHistogram", func(t *testing.T) {
		meas, comp := Builder[int64]{
			Temporality: metricdata.DeltaTemporality,
		...
		meas(ctx, largeVal, alice)
		require.Equal(t, 1, comp(&agg))
		hist := agg.(metricdata.ExponentialHistogram[int64])
		require.Len(t, hist.DataPoints, 1)
		assert.Equal(t, largeVal, hist.DataPoints[0].Sum)
	...
	t.Run("CumulativeExponentialHistogram", func(t *testing.T) {
		meas, comp := Builder[int64]{
			Temporality: metricdata.CumulativeTemporality,
		}.ExponentialBucketHistogram(
			160,
