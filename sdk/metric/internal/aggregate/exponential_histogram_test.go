...
func testExpoHistogramDataPointRecord[N int64 | float64](t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(fmt.Sprint(tt.values), func(t *testing.T) {
			...
			dp := newExpoHistogramPointCounters[N](tt.maxSize, 20)
...
func testExpoHistogramDataPointRecordFloat64(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(fmt.Sprint(tt.values), func(t *testing.T) {
			...
			dp := newExpoHistogramPointCounters[float64](tt.maxSize, 20)
			for _, v := range tt.values {
				dp.recordCount(v)
				dp.recordCount(-v)
			}
...
func TestExponentialHistogramDataPointRecordLimits(t *testing.T) {
	...
	fdp := newExpoHistogramPointCounters[float64](4, 20)
	...
	idp := newExpoHistogramPointCounters[int64](4, 20)
...
func BenchmarkPrepend(b *testing.B) {
	for i := 0; i < b.N; i++ {
		agg := newExpoHistogramPointCounters[float64](1024, 20)
		n := math.MaxFloat64
		for range 1024 {
			agg.recordCount(n)
			n /= 2
...
func TestSubNormal(t *testing.T) {
	...
	ehdp := newExpoHistogramPointCounters[float64](4, 20)
