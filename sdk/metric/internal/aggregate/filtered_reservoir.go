...
type FilteredExemplarReservoir[N int64 | float64] interface {
	...
	Offer(ctx context.Context, val N, lazyDropped LazyAttributes)
...
func (f *filteredExemplarReservoir[N]) Offer(ctx context.Context, val N, lazyDropped LazyAttributes) {
	if f.filter(ctx) {
		...
		if lr, ok := f.reservoir.(interface {
			OfferLazy(context.Context, time.Time, exemplar.Value, LazyAttributes)
		}); ok {
			lr.OfferLazy(ctx, ts, exemplar.NewValue(val), lazyDropped)
		} else {
			f.reservoir.Offer(ctx, ts, exemplar.NewValue(val), lazyDropped.Get())
		}
	}
}
