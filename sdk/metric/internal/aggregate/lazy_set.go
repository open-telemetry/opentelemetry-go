...
func newLazyFilteredSet(set attribute.Set, filter attribute.Filter) lazyFilteredSet {
	if filter == nil || set.Len() == 0 {
		return lazyFilteredSet{orig: set, distinct: set.Equivalent()}
	}

	if set.Len() > 64 {
		filteredKVs := make([]attribute.KeyValue, 0, set.Len())
		var droppedKVs []attribute.KeyValue
		iter := set.Iter()
		for iter.Next() {
			kv := iter.Attribute()
			if filter(kv) {
				...
				droppedKVs = append(droppedKVs, kv)
			}
		}
		if len(droppedKVs) == 0 {
			return lazyFilteredSet{
				orig:     set,
				distinct: set.Equivalent(),
			}
		}
		filtered := attribute.NewSet(filteredKVs...)
		return lazyFilteredSet{
			orig:     set,
			distinct: filtered.Equivalent(),
			fallback: &filtered,
			dropped:  droppedKVs,
		}
	}
