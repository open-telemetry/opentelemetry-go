...
func (s *sumValueMap[N]) measure(
	...
	sv := s.values.LoadOrStoreAttr(lazySet, func(attr attribute.Set) *sumValue[N] {
		r := s.newRes(attr)
	...
	if !sv.dropExemplars {
		sv.res.Offer(ctx, value, lazySet)
	}
}
...
func (s *deltaSum[N]) measure(ctx context.Context, value N, lazySet lazyFilteredSet) {
	hotIdx := s.hcwg.start()
