...
func TestExtractValidTraceContextFromHTTPReq(t *testing.T) {
	props := otel.New(otel.WithExtractors(propagators.TraceContext{}))
	tests := []struct {
		name   string
		header string
...
func TestInjectTraceContextToHTTPReq(t *testing.T) {
	...
	props := otel.New(otel.WithInjectors(propagators.TraceContext{}))
	tests := []struct {
		name       string
...
func TestTraceStatePropagation(t *testing.T) {
	props := otel.New(otel.WithInjectors(propagators.TraceContext{}), otel.WithExtractors(propagators.TraceContext{}))
	want := "opaquevalue"
