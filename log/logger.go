...
type Logger interface {
	...
	// opts.
	//
	// The passed opts is likely to be a partial record with only the
	// bridge-relevant information being provided (e.g a opts with only the
	...
	// provided context and opts, and will be false if the Logger will not
	...
	//
	// The opts should not be held by the implementation. A copy should be
	// made if the record needs to be held after the call returns.
	//
	// Implementations of this method need to be safe for a user to call
	// concurrently.
	Enabled(ctx context.Context, opts EnabledOpts) bool
}
