...
- Add `String` method to `attribute.Value` in `go.opentelemetry.io/otel/attribute` implementing `fmt.Stringer`; deprecate `Emit` in favor of `String`; update `go.opentelemetry.io/otel/exporters/prometheus` to use `Value.String`. (#8154)
