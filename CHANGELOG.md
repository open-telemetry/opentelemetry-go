...
- Remove the `grpc.WithBlock()` attribute used in `TestDisconnected` function with `client_test.go` as well as `otpl_integration_test.go` to ensure that `make test` could run properly. (#1951)
