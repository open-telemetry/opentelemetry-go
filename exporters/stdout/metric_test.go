...
func TestStdoutTimestamp(t *testing.T) {
	...
	ctx := context.Background()
	desc := otel.NewDescriptor("test.name", otel.ValueObserverKind, otel.Int64NumberKind)
...
func TestStdoutCounterFormat(t *testing.T) {
	...
	desc := otel.NewDescriptor("test.name", otel.CounterKind, otel.Int64NumberKind)
...
func TestStdoutLastValueFormat(t *testing.T) {
	...
	desc := otel.NewDescriptor("test.name", otel.ValueObserverKind, otel.Float64NumberKind)
	lvagg, ckpt := metrictest.Unslice2(lastvalue.New(2))
...
func TestStdoutMinMaxSumCount(t *testing.T) {
	...
	desc := otel.NewDescriptor("test.name", otel.ValueRecorderKind, otel.Float64NumberKind)
