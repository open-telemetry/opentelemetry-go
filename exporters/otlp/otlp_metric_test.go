...
var (
	...
	intCounterMetadata = &metricpb.MetricDescriptor{
		Name:      "int64-count",
				...
				InstrumentationLibraryMetrics: []*metricpb.InstrumentationLibraryMetrics{
					{
						Metrics: []*metricpb.Metric{
							{
								MetricDescriptor: intCounterMetadata,
...
func TestValueRecorderInt64MetricGroupingExport(t *testing.T) {
	r := record{
		"valuerecorder",
...
func TestValueRecorderFloat64MetricGroupingExport(t *testing.T) {
	r := record{
		"valuerecorder",
		metric.ValueRecorderKind,
	...
	for _, rm := range expected {
		for _, ilm := range rm.InstrumentationLibraryMetrics {
			...
			for i, expected := range ilm.GetMetrics() {
				...
				assert.ElementsMatch(t, expected.GetPoints(), g[i].GetPoints())
			}
		}
	}
	for k := range got {
