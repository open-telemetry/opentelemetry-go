	...
	tests := []struct {
	...
	}{
		{
			...
			&metricpb.MetricDescriptor{
				...
				ValueType:   metricpb.MetricDescriptor_SUMMARY_INT64,
			},
		},
		{
			"mmsc-test-b",
			...
			&metricpb.MetricDescriptor{
				...
				ValueType:   metricpb.MetricDescriptor_SUMMARY_DOUBLE,
			},
		},
	}
...
func TestMinMaxSumCountDatapoints(t *testing.T) {
	desc := metric.NewDescriptor("desc", metric.ValueRecorderKind, metric.Int64NumberKind)
	labels := label.NewSet(kv.Int("A", 1))
	...
	if assert.NoError(t, err) {
		assert.Equal(t, expected, m.Points)
	}
}
	...
	tests := []struct {
	...
	}{
		{
			...
			&metricpb.MetricDescriptor{
				...
				Unit:        "1",
				Kind:        metricpb.MetricDescriptor_ADDING_MONOTONIC_DELTA_SYNCHRONOUS,
				ValueType:   metricpb.MetricDescriptor_SCALAR_INT64,
			},
		},
		{
			"sum-test-b",
			...
			&metricpb.MetricDescriptor{
				...
				Unit:        "ms",
				Kind:        metricpb.MetricDescriptor_GROUPING_DELTA_SYNCHRONOUS,
				ValueType:   metricpb.MetricDescriptor_SCALAR_DOUBLE,
			},
		},
	}
...
func TestSumInt64DataPoints(t *testing.T) {
	...
	if m, err := sum(record, ckpt.(aggregation.Sum)); assert.NoError(t, err) {
		assert.Equal(t, []*metricpb.DataPoint{{
			StartTimeUnixNano: uint64(intervalStart.UnixNano()),
			TimeUnixNano:      uint64(intervalEnd.UnixNano()),
			ValueInt64:        1,
		}}, m.Points)
...
func TestSumFloat64DataPoints(t *testing.T) {
	...
	labels := label.NewSet()
	s, ckpt := test.Unslice2(sumAgg.New(2))
	assert.NoError(t, s.Update(context.Background(), metric.NewFloat64Number(1.5), &desc))
	require.NoError(t, s.SynchronizedMove(ckpt, &desc))
	record := export.NewRecord(&desc, &labels, nil, ckpt.Aggregation(), intervalStart, intervalEnd)
	if m, err := sum(record, ckpt.(aggregation.Sum)); assert.NoError(t, err) {
		assert.Equal(t, []*metricpb.DataPoint{{
