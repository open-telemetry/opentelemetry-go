...
func newExporterEndToEndTest(t *testing.T, additionalOpts []otlp.ExporterOption) {
	...
	for _, m := range metrics {
		...
		case metric.CounterKind:
			switch data.nKind {
			case metricapi.Int64NumberKind:
				assert.Equal(t, metricpb.MetricDescriptor_SCALAR_INT64.String(), desc.GetValueType().String())
				if dp := m.GetPoints(); assert.Len(t, dp, 1) {
					assert.Equal(t, data.val, dp[0].ValueInt64, "invalid value for %q", desc.Name)
				}
			case metricapi.Float64NumberKind:
				assert.Equal(t, metricpb.MetricDescriptor_SCALAR_DOUBLE.String(), desc.GetValueType().String())
				if dp := m.GetPoints(); assert.Len(t, dp, 1) {
		...
		case metric.ValueRecorderKind, metric.ValueObserverKind:
			switch data.nKind {
			case metricapi.Int64NumberKind:
				assert.Equal(t, metricpb.MetricDescriptor_SUMMARY_INT64.String(), desc.GetValueType().String())
				if dp := m.GetPoints(); assert.Len(t, dp, 1) {
					count := dp[0].Summary.Count
					assert.Equal(t, uint64(1), count, "invalid count for %q", desc.Name)
					assert.Equal(t, data.val*int64(count), dp[0].Summary.SumInt64, "invalid sum for %q (value %d)", desc.Name, data.val)
				}
			case metricapi.Float64NumberKind:
				assert.Equal(t, metricpb.MetricDescriptor_SUMMARY_DOUBLE.String(), desc.GetValueType().String())
				if dp := m.GetPoints(); assert.Len(t, dp, 1) {
					count := dp[0].Summary.Count
					assert.Equal(t, uint64(1), count, "invalid count for %q", desc.Name)
					assert.Equal(t, float64(data.val*int64(count)), dp[0].Summary.SumDouble, "invalid sum for %q (value %d)", desc.Name, data.val)
				}
			default:
				assert.Failf(t, "invalid number kind", data.nKind.String())
			}
