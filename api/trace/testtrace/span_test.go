...
func TestSpan(t *testing.T) {
	...
	t.Run("#Attributes", func(t *testing.T) {
		...
		t.Run("cannot be changed after the span has been ended", func(t *testing.T) {
			...
			subject.SetAttribute(expectedAttr)
			subject.End()

			unexpectedAttr := expectedAttr.Key.String("unexpected")
			subject.SetAttribute(unexpectedAttr)
	...
	t.Run("#Links", func(t *testing.T) {
		...
		t.Run("returns the most recently set links", func(t *testing.T) {
			t.Parallel()
			...
			_, linkedSpan1 := tracer.Start(context.Background(), "link1")
			_, linkedSpan2 := tracer.Start(context.Background(), "link1")

			link1Attr1 := core.Key("key1").String("value1")
			link1Attr2 := core.Key("key2").String("value2")
			link2Attr := core.Key("key3").String("value3")
			unexpectedAttr := core.Key("key4").String("unexpected")

			subject.Link(linkedSpan1.SpanContext(), unexpectedAttr)
			subject.Link(linkedSpan1.SpanContext(), link1Attr1, link1Attr2)
			subject.AddLink(trace.Link{
				SpanContext: linkedSpan2.SpanContext(),
				Attributes:  []core.KeyValue{link2Attr},
			})

			links := subject.Links()

			e.Expect(links[linkedSpan1.SpanContext()]).ToMatchInAnyOrder([]core.KeyValue{link1Attr1, link1Attr2})
			e.Expect(links[linkedSpan1.SpanContext()]).NotToContain(unexpectedAttr)
			e.Expect(links[linkedSpan2.SpanContext()]).ToContain(link2Attr)
		})
