...
//
// If SingleHeader is set to true then X-B3 header is used to inject
// and extract. Otherwise, separate headers are used to inject and
// extract.
type B3Propagator struct {
	SingleHeader bool
}

var _ apipropagation.SpanContextPropagator = B3Propagator{}

func (b3 B3Propagator) Inject(sc core.SpanContext, supplier apipropagation.Supplier) {
	if sc.IsValid() {
		if b3.SingleHeader {
...
func (b3 B3Propagator) extract(supplier apipropagation.Supplier) core.SpanContext {
	...
	debug, ok := b3.extractDebugFlag(supplier.Get(B3DebugFlagHeader))
	if !ok {
		return core.EmptySpanContext()
	}
...
// extractDebugFlag parses the value of the X-B3-Sampled b3Header.
func (b3 B3Propagator) extractDebugFlag(debug string) (flag byte, ok bool) {
	switch debug {
	case "", "0":
		return 0, true
	case "1":
