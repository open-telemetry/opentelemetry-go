...
func (e *Expectation) ToBeTrue() {
	...
	default:
		e.fail("Cannot check if non-bool value\n\t%v\nis truthy", a)
	}
}
...
func (e *Expectation) NotToPanic() {
	...
	default:
		e.fail("Cannot check if non-func value\n\t%v\nis truthy", a)
	}
}
...
func (e *Expectation) ToSucceed() {
	...
	case error:
		if actual != nil {
			e.fail("Expected error\n\t%v\nto have succeeded", actual)
		}
	default:
		e.fail("Cannot check if non-error value\n\t%v\nsucceeded", actual)
	}
}
...
func (e *Expectation) ToBeTemporally(matcher TemporalMatcher, compareTo interface{}) {
	if actual, ok := e.actual.(time.Time); ok {
		...
		if !ok {
			e.fail("Cannot compare to non-temporal value\n\t%v", compareTo)
			return
		}
	...
	e.fail("Cannot compare non-temporal value\n\t%v", e.actual)
}
...
func (e *Expectation) fail(format string, a ...any) {
