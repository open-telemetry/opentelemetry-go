...
type UniqueInstrumentMeterImpl struct {
	...
	state map[string]sdkapi.Instrument
}
...
func (u *UniqueInstrumentMeterImpl) checkUniqueness(descriptor sdkapi.Descriptor) (sdkapi.Instrument, error) {
...
// NewInstrument implements sdkapi.MeterImpl.
func (u *UniqueInstrumentMeterImpl) NewInstrument(descriptor sdkapi.Descriptor) (sdkapi.Instrument, error) {
	u.lock.Lock()
	defer u.lock.Unlock()
	...
	if err != nil {
		...
		return impl.(sdkapi.Instrument), nil
	}

	syncInst, err := u.impl.NewInstrument(descriptor)
	if err != nil {
		return nil, err
	}
