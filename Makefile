...
semconv-generate: | $(SEMCONVGEN) $(SEMCONVKIT)
	...
	@$(SEMCONVGEN) -i "$(OTEL_SPEC_REPO)/semantic_conventions" --only=span -f trace.go -t "$(SEMCONVPKG)/template.j2" -s "$(TAG)"
	@$(SEMCONVGEN) -i "$(OTEL_SPEC_REPO)/semantic_conventions" --only=resource -f resource.go -t "$(SEMCONVPKG)/template.j2" -s "$(TAG)"
