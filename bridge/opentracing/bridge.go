...
func (s *bridgeSpan) FinishWithOptions(opts ot.FinishOptions) {
	...
	if !opts.FinishTime.IsZero() {
		otelOpts = append(otelOpts, otel.WithEndTime(opts.FinishTime))
	}
...
func otLogFieldsToOtelCoreKeyValues(fields []otlog.Field) []otel.KeyValue {
...
func (t *BridgeTracer) StartSpan(operationName string, opts ...ot.StartSpanOption) ot.Span {
	...
	checkCtx2, otelSpan := t.setTracer.tracer().Start(checkCtx, operationName, func(opts *otel.SpanOptions) {
...
func otTagsToOtelAttributesKindAndError(tags map[string]interface{}) ([]otel.KeyValue, otel.SpanKind, bool) {
	kind := otel.SpanKindInternal
	error := false
	var pairs []otel.KeyValue
	for k, v := range tags {
		switch k {
		case string(otext.SpanKind):
			if s, ok := v.(string); ok {
...
func otTagToOtelCoreKeyValue(k string, v interface{}) otel.KeyValue {
...
func otTagToOtelCoreKey(k string) otel.Key {
	return otel.Key(k)
}

type bridgeRelation struct {
	spanContext      *bridgeSpanContext
	relationshipType otel.RelationshipType
}

func (r bridgeRelation) ToOtelRelation() otel.Relation {
	if r.spanContext == nil {
		return otel.Relation{}
	}
	return otel.Relation{
...
func otSpanReferenceTypeToOtelRelationshipType(srt ot.SpanReferenceType) otel.RelationshipType {
	switch srt {
	case ot.ChildOfRef:
		return otel.ChildOfRelationship
	case ot.FollowsFromRef:
		return otel.FollowsFromRelationship
	default:
...
func traceFlagsToString(opts byte) string {
	var parts []string
	if opts&otel.TraceFlagsSampled == otel.TraceFlagsSampled {
...
func (t *BridgeTracer) Extract(format interface{}, carrier interface{}) (ot.SpanContext, error) {
	...
	err := hhcarrier.ForeachKey(func(k, v string) error {
		...
		switch ck {
		case traceIDHeader:
			traceID, err := otel.TraceIDFromHex(v)
			if err != nil {
				return err
			}
			bridgeSC.otelSpanContext.TraceID = traceID
		case spanIDHeader:
			spanID, err := otel.SpanIDFromHex(v)
			if err != nil {
				return err
			}
