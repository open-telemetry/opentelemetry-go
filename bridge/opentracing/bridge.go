...
func (t *BridgeTracer) NewHookedContext(ctx context.Context) context.Context {
	ctx = otelcorrelation.WithSetHook(ctx, t.correlationSetHook)
	ctx = otelcorrelation.WithGetHook(ctx, t.correlationGetHook)
	return ctx
}
...
func (t *BridgeTracer) correlationSetHook(ctx context.Context) context.Context {
	...
	// during FromContext, but otherwise we don't change the
	// context, so we don't care about the old hooks.
	clearCtx, _, _ := otelcorrelation.WithNoHooks(ctx)
	m := otelcorrelation.FromContext(clearCtx)
