...
func Value(ctx context.Context, key label.Key) label.Value {
	v, _ := updatable.MapFromContext(ctx).Value(key)
	return v
}
...
func ContextWithValues(parent context.Context, pairs ...label.KeyValue) context.Context {
	m := updatable.MapFromContext(parent).Apply(updatable.MapUpdate{
		MultiKV: pairs,
	})
	return updatable.ContextWithMap(parent, m)
}
...
func ContextWithEmpty(parent context.Context) context.Context {
	return updatable.ContextWithNoCorrelationData(parent)
}
